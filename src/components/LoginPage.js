...
const LoginPage = () => {
  const { loginWithRedirect, isLoading: auth0Loading, error: auth0Error } = useAuth0();
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="max-w-md w-full space-y-8 p-8 bg-white rounded-lg shadow">
        <div className="text-center">
          ...
          {auth0Error && (
            <p className="mt-2 text-red-600">
              {auth0Error?.message || 'Ошибка аутентификации'}
            </p>
          )}
        </div>
        ...
        >
          ...
          {auth0Loading ? 'Выполняется вход...' : 'Войти через Google'}
        </button>
      </div>
    </div>
  );
